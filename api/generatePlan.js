...
function generateItinerary(dest, days, perDayCost, route) {
  ...
  const formatMinuteRange = (minStart, durationMin = 45) => {
  ...
  for (let index = 0; index < days; index += 1) {
    ...
    if (days === 1) {
      activities.push(`Start early from ${primaryLeg?.from || "your city"} (~06:00 – 06:45 AM)`);
      activities.push(...dayAttractions.map((attraction) => `Visit ${attraction}`));
      activities.push(`Return travel back home (~06:30 – 07:30 PM)`);

      timeSchedule.push(
        { time: formatMinuteRange(360, 45), activity: `Depart from ${primaryLeg?.from || "your city"}`, category: "travel", status: primaryLeg?.confidence || "estimated", order: 360 },
      ...
      if (isFirstDay) {
        if (isOvernightTrip) {
          activities.push(`Arrive in ${dest.name} (~05:30 – 06:30 AM) via ${route.map((leg) => `${leg.serviceName || leg.mode}`).join(" + ")}`);
          ...
          timeSchedule.push(
            { time: "05:30 – 06:30 AM", activity: `Arrive in ${dest.name}`, category: "travel", status: primaryLeg?.confidence || "verified", order: 330 },
            { time: "07:30 – 08:30 AM", activity: "Hotel luggage drop & fresh up", category: "stay", status: "grounded", order: 450 },
            { time: "08:30 – 09:30 AM", activity: "Traditional South Indian breakfast", category: "food", status: "grounded", order: 510 },
          ...
          dayAttractions.forEach((spot, i) => {
            ...
            const timing = isTemple ? (i === 0 ? "09:30 – 11:30 AM" : "05:00 – 06:30 PM") : (i === 0 ? "10:00 AM – 12:30 PM" : i === 1 ? "02:30 – 04:30 PM" : "05:00 – 06:30 PM");
            const orderMin = isTemple ? (i === 0 ? 570 : 1020) : (i === 0 ? 600 : i === 1 ? 870 : 1020);
